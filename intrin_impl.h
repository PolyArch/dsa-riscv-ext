 ...
 */
inline void SS_SET_ACCEL(REG x) {
  SS_CONTEXT(1ull << (x));
}
...
inline void SS_REPEAT_PORT(int port, REG n) {
  SS_CONFIG_PORT(port, PortField::PortRepeat, n);
}
 ...
 */
inline void CONFIG_1D_STREAM(REG start, uint64_t length) {
...
inline void INSTANTIATE_1D_STREAM(REG addr, REG bytes,
  ...
  int64_t length_ = bytes_ / (wbytes);
  CONFIG_1D_STREAM(addr, length_);
  auto value =
          LINEAR_STREAM_MASK(port, padding, action, /*1d*/0, operation, memory, signal);
